...
class Students extends Component
{
    ...
    //                 }
    //             }
    //             return $x + ($studying->addional_grades ?? 0) + ($studying->midterm_exam ?? 0) + ($studying->final_exem ?? 0);
    //         case 'persents':
    //             $x = 0;
    //             for ($i = 1; $i < 16; $i++) {
    //                 switch ($studying->{'persents' . $i}) {
    ...
    //             case 'midterm_exam':
    //                 return $studying->midterm_exam ?? 'لم يتم رصد البيانات';
    ...
    public function select($id)
    {
        ...
        $this->midterm_exam = $this->student_selected->helf_grade;
    ...
    {
        $this->validate([
            'midterm_exam' => 'required|numeric|min:0',
            'working' => 'required|numeric|min:0',
        ]);
        // dd($this->student_selected, $this->midterm_exam, $this->working);
        // create or update
        if (
            $this->midterm_exam + $this->working +
        ...
        Studying::updateOrCreate([
            ...
            'midterm_exam' => $this->midterm_exam,
