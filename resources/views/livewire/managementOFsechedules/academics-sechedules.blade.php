@section('nav')
    @livewire('components.nav.management_o_f_sechedules.academics-sechedules-header', ['department' => $department, 'active' => $active, 'terms' => $terms, 'types' => $types])
@endsection
<div>
                ...
                <tbody>
                    @forelse ($academics as $academic)
                        <tr class="table-light" id="modldetials" style="margin-top:7px;">
                            <td><button type="submit" class="btn btn-primary btn-sm btn_edit"
                                    id="{{ $academic->user_id }}d" data-toggle="modal"
                                    data-target="#MessageApprovementDeleteModal"
                                    wire:click='selectedSchedule({{ $academic->user_id }},{{ true }})'
                                    @if (!$academic->$nameSchedule) disabled @endif>حذف الجدول <img
                                        src="{{ Vite::image('delete (1).png') }}" id=""
                                        width="15px"></button> </td>
                            <td><button type="submit" class="btn btn-primary btn-sm btn_edit"
                                    id="{{ $academic->user_id }}v" data-toggle="modal" data-target="#UploadeFileModal"
                                    wire:click='uploadSchedule({{ $academic->user_id }})'>رفع جدول <img
                ...
                <div class="modal-body modal_body_css ">
                    <form action="" style="display: block;">
                        <div class="form-group">
                            ...
                            <input type="file" class="form-control-file border" id="file" wire:model="schedule"
                                style="height: 30px; margin-top:8px" accept=".png,.jpg,.jpeg">
                        </div>
                        <!-- <div class="form-group">
