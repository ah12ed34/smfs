@section('nav')
    @livewire('components.nav.management_o_f_sechedules.students-sechedules-header', ['level' => $level, 'department' => $department])
@endsection
<div>
                    ...
                    @forelse ($groups as $group)
                        <tr class="table-light" id="modldetials" style="margin-top:7px;">
                            <td><button type="submit" class="btn btn-primary btn-sm btn_edit" id=""
                                    ...
                                    wire:click='selectedSchedule({{ $group->id }},true)'
                                    @if (!$group->schedule) disabled @endif>حذف الجدول <img
                                        src="{{ Vite::image('delete (1).png') }}" id=""
                                        width="15px"></button> </td>
                            <td><button type="submit" class="btn btn-primary btn-sm btn_edit" id=""
                                    data-toggle="modal" data-target="#UploadeFileModal"
                                    wire:click='uploadSchedule({{ $group->id }})'>رفع جدول <img
                                        src="{{ Vite::image('upload.png') }}" id="" width="15px"></button>
                            </td>
                            <td><button type="submit" class="btn btn-primary btn-sm btn_detials" id=""
                                    data-toggle="modal" data-target="#DisplaySeheduleModal"
                                    wire:click='showSchedule({{ $group->id }})'
                                    @if (!$group->schedule) disabled @endif>عرض الجدول</button> </td>
            ...
            <div class="modal-content UploadeFileModal" id="modal-content2" style="height: 250px;">
                    ...
                    <form action="" style="display: block;">
                        <div class="form-group">
                            <input type="file" class="form-control-file border" id="file"
                                style="height: 30px; margin-top:8px" wire:model='schedule' accept='.png,.jpg,.jpeg'>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary btn-sm btn_save_informModal" id=""
                        wire:click='uploadScheduleSave'>حفظ</button>
            ...
            <div class="modal-content ModaldDetailsAcademic" id="modal-content"
                ...
                <div class="modal-body ModaldDetailsAcademic">
                    @if ($openType == 'show' && $academicData->schedule)
                        <img class="img-fluid" src="{{ $academicData->schedule }}" id="studying-schedule"
                            height="100%">
                    @else
                        <div class="alert alert-danger" role="alert">
