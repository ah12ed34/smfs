                    ...
                    <thead class="table-header">
                        ...
                        <tr class="table-light" id="modldetials" style="font-size:10px; margin-left:5px; ">
                            ...
                            <th colspan="5 " rowspan="2"
                                ...
                                <label style="margin-top:10px;">اسم الطالب</label>
                            </th>
                            <th colspan="5 " rowspan="2"
                                style="padding-left:40px;padding-right:40px;padding-top:10px; "> <label
                                    style="margin-top:10px;">الرقم الأكاديمي</label></th>
                            ...
                            <tr class="table-light" id=" ">
                                @for ($i = 12; $i > 0; $i--)
                                    <td><label class="persent">
                                            <input type="radio" id="{{ $i . $student->user_id }}" value="present"
                                                wire:model="lecture.{{ $student->user_id . '.' . $i }}" />
                                        </label> </td>
                                    <td><label class="late">
                                            <input type="radio" id="{{ $i . $student->user_id }}" value="late"
                                                wire:model="lecture.{{ $student->user_id . '.' . $i }}" />
                                        </label> </td>

                                    <td> <label class="permit">
                                            <input type="radio" id="{{ $i . $student->user_id }}" value="permit"
                                                wire:model="lecture.{{ $student->user_id . '.' . $i }}" />
                                        </label></td>

                                    <td> <label class="absent">
                                            <input type="radio" id="{{ $i . $student->user_id }}" value="absent"
                                                wire:model="lecture.{{ $student->user_id . '.' . $i }}" />
                                        </label></td>
                                @endfor
