                    ...
                    @forelse($subjects as $subject)
                        ...
                        <tr>
                            <td><button type="submit" class="btn btn-primary btn-sm btn_edit" id=""
                                    data-toggle="modal" data-target="#EditeSubjectModal">تعديل<img
                                        src="{{ Vite::image('edit.png') }}" id="" width="15px"></button>
                            </td>
                            <td><input type="checkbox" wire:model.lazy='isActivated.{{ $subject->id }}'
                                    id='isActivated.{{ $subject->id }}'></td>
                            <td><input type="checkbox" wire:model.lazy='has_practical.{{ $subject->id }}'
                                    id='has_practical.{{ $subject->id }}'></td>
                            <td><input type="number" value="1" wire:model='semester.{{ $subject->id }}'
                                    id='semester.{{ $subject->id }}' placeholder="semester"></td>
                            <td>
                                @if ($subject->has_practical || (isset($has_practical[$subject->id]) && $has_practical[$subject->id]))
                                    <input type="number" wire:model='practical_grade.{{ $subject->id }}'
                                        id='practical_grade{{ $subject->id }}' placeholder="درجة العملي">
                                @endif
                            </td>
                            ...
                            <td>
                                <input type="checkbox" id='{{ $subject->id }}'
                                    wire:model.lazy='select.{{ $subject->id }}' />
                            </td>

                        </tr>
                    @empty
                        <tr>
                            <td colspan="9">{{ __('general.no_data') }}</td>
                        </tr>
                    @endforelse
                    <tr>
                        <td colspan="9"><button type="submit"
                                class="btn btn-primary">{{ __('general.add') }}</button></td>
                    </tr>
                </form>
