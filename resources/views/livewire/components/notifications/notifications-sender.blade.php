            ...
            <div class="modal-content modal_content_css" id="modal-content" style="background-color: #F6F7FA;height:95vh;">
                ...
                <div class="modal-body modal_body_chattinbox" id="">
                    <div class="card card-light header_sendNotifications">
                        @if (in_array('department', $selectActive))
                            <div class="dropdown">
                                ...
                                <button id="" type="button"
                                    class="btn btn-light specific_department dropdown-toggle" data-toggle="dropdown"
                                    dir="rtl">
                                    {{-- جميع الأقسام --}}
                                ...
                                <div id="dropdown-menulist" class="dropdown-menu">
                                    ...
                                    @foreach ($departments->where('id', '!=', $active['department']) as $department)
                                        <a id="dropdown-students-itemlist" class="dropdown-item"
                                            ...
                                            style="padding-left:40px;">{{ $department->name }}</a>
                                    @endforeach
                        ...
                        @if (in_array('role', $selectActive))
                            <div class="dropdown">
                                ...
                                <button id="" type="button"
                                    class="btn btn-light specific_department dropdown-toggle" data-toggle="dropdown"
                                    dir="rtl">
                                    {{ $active['role'] != null ? $roles->where('id', $active['role'])->first()->description : 'جميع الموظفين' }}
                                </button>
                                <div id="dropdown-menulist" class="dropdown-menu">
                                    @if ($active['role'] != null)
                                        <a id="dropdown-students-itemlist" class="dropdown-item"
                                            wire:click="setRole(null)" style="padding-left:40px;">
                                            جميع الموظفين
                                        </a>
                                    @endif
                                    @foreach ($roles->where('id', '!=', $active['role']) as $role)
                                        <a id="dropdown-students-itemlist" class="dropdown-item"
                                            ...
                                            style="padding-left:40px;">{{ $role->description }}</a>
                                    @endforeach
                                </div>
                        ...
                        @if (in_array('group', $selectActive))
                            <div class="dropdown">
                                <button id="" type="button"
                                    class="btn btn-light specific_group dropdown-toggle"
                                    style="float: left;
                                position: relative;left: 10%;"
                                    @if ($active['level'] == null && in_array('level', $selectActive)) disabled @endif data-toggle="dropdown"
                                    dir="rtl">
                                    {{-- @dd($active['group'], $groups->where('id', $active['group'])->first()) --}}
                                    {{ $active['group'] != null ? $groups->where('id', $active['group'])->first()?->name : 'جميع المجموعات' }}
                                </button>
                                <div id="dropdown-menulist" class="dropdown-menu">
                        ...
                        @if (in_array('level', $selectActive))
                            <div class="dropdown">
                                <button id="" type="button"
                                    class="btn btn-light specific_group dropdown-toggle"
                                    @if ($active['department'] == null && in_array('department', $selectActive)) disabled @endif data-toggle="dropdown"
                                    dir="rtl">
                                    {{ $active['level'] != null ? $levels->where('id', $active['level'])->first()?->name : 'جميع المستويات' }}
                                </button>
                                <div id="dropdown-menulist" class="dropdown-menu">
                                    ...
                                    @foreach ($levels->where('id', '!=', $active['level']) as $level)
                                        <a id="dropdown-students-itemlist" class="dropdown-item"
                                            wire:click="setLevel({{ $level->id }})"
                                            style="padding-left:40px;">{{ $level->name }}</a>
                                    @endforeach
                                    {{-- <a id="dropdown-students-itemlist" class="dropdown-item" href="#"
                                    style="padding-left:40px;">(1)المجموعة</a>
                                <a id="dropdown-students-itemlist" class="dropdown-item" href="#"
                            ...
                            <div class="card-body chatbox " id="chatbox">
                                ...
                                @forelse ($notifications as $notification)
                                    <div class="message sender-message">
                                        <img src="{{ Vite::image('user.png') }}" alt="User Profile"
                                            class="profile-pic">
                                        <div class="message-content">
                                            ...
                                            <div class="message-footer">
                                                <span
                                                    class="time">{{ $notification->created_at->format('h:i A') }}</span>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="modal-footer" style="z-index: 1; ">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="text"
                            class="form-control send-input @if ($errors->has('message')) is-invalid @endif"
                            id="messageInput"
                            placeholder="@if ($errors->has('message')) {{ $errors->first('message') }}@else اكتب... @endif"
