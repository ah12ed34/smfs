...
@section('nav')
    <div class="hdr2" style=" box-shadow: 10px;">
        <button class=" spaces" onclick="window.location='{{ route('admin.department') }}'"> <label class="subjectname"> الأدمن
            </label><img src="{{ Vite::image('left-arrow.png') }}" id="subject-icon-hdr2" width="40px">
        </button>
...
@endforelse --}}
        ...
        @forelse ($levels as $level)
            <div class="card  cards-departments depart-level-quality" id=""
                onclick="window.location='{{ route('admin.students_data', $level->id) }}' ">
                {{-- loop intger --}}
                @if (4 <= $loop->index)
                    <img src="{{ Vite::image('level4.png') }}" class="" width="150px">
                @else
                    <img src="{{ Vite::image('level' . ($loop->index + 1) . '.png') }}" class="" width="150px">
                @endif
