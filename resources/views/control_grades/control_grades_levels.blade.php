...
@section('nav')
    <div class="hdr2" style=" box-shadow: 10px;">
        <button class=" spaces" onclick="window.location='{{ route('control_grades_departments') }}'"> <label
                class="subjectname"> الكنترول </label><img src="{{ Vite::image('left-arrow.png') }}" id="subject-icon-hdr2"
                width="40px">
        </button>
...
@section('content')
    <div class="container" style="padding-top: 20px;">
        @foreach ($levels as $level)
            <div class="card  cards-departments depart-level-quality" id="{{ $loop->index }}"
                onclick="window.location='{{ route('control_grades_main', $level->id) }}'">
                @if (4 < $loop->index + 1)
